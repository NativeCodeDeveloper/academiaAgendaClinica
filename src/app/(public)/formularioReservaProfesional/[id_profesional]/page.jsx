    ...
    async function agendarSinPago(
        ...
        try {
            ...
            if (!nombrePaciente || !apellidoPaciente || !rut || !telefono || !email  || !id_profesional ) {
                return toast.error('Debe completar toda la informacion');

            }

            if(!servicioSeleccionado || !tarifaSeleccionadaIndex){
                return toast.error('Debe Seleccionar el Servicio');
            }


            if(!fechaInicio || !horaInicio || !horaFinalizacion){
                return toast.error('Debe seleccionar un dia y hora de atención. (Vuelva a pestaña anterior y seleccione nuevamente)');
            }
            ...
            const res = await fetch(`${API}/reservaPacientes/insertarReservaPacienteFicha`, {
                ...
                body: JSON.stringify({
                    ...
                    fechaFinalizacion,
                    horaFinalizacion,
                    monto_reserva : tarifaSeleccionadaIndex,
                    motivo_reserva : servicioSeleccionado,
                    estadoReserva: "reservada" ,
                    id_profesional})
            });
